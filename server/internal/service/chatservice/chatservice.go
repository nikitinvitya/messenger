...
type ChatService interface {
	...
	ListUserChats(ctx context.Context, userID int) ([]model.ChatResponse, error)
...
func (s *chatService) ListUserChats(ctx context.Context, userID int) ([]model.ChatResponse, error) {
	...
	for _, chat := range chats {
		...
		fullChatInfo := model.ChatResponse{
			ID:           chat.ID,
			Name:         chat.Name,
			Type:         chat.Type,
